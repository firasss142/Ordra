-- ============================================================
-- 20260823000004_dashboard_carriers_live.sql
-- Dashboard round-4: the carrier block becomes the whole lower page.
--
-- Rounds 1-3 left /dashboard with three sections; the user then removed the
-- product, flow and queue blocks, so Transporteurs now owns that space and has
-- to earn it. The redesign is a hero donut (market-wide delivery rate) over a
-- comparable carrier table, and the table gains one genuinely new column:
-- what each carrier is holding RIGHT NOW.
--
-- WHAT CHANGES HERE:
--
--   carrier_live   (new)  In-flight and stuck parcel counts per carrier. The
--                         status list and the 3-day stuck threshold are lifted
--                         verbatim from the three existing routes that already
--                         define them, so the dashboard cannot disagree with
--                         /suivi-transporteur about what "bloquee" means.
--
--   'carriers'     json    Now built from the UNION of carriers_agg (90-day
--                          resolved history) and carrier_live (right now).
--                          Previously a carrier with stuck parcels but no
--                          completed delivery in the window was invisible.
--
-- THE SIGNATURE IS UNCHANGED, so this is CREATE OR REPLACE with no DROP. v2
-- needed the drop only because it added p_carrier_from; dropping here would
-- open a window where PostgREST resolves no function at all.
--
-- Deliberately NOT included: a true-cost-per-delivered metric folding in
-- carriers.return_fee. It was measured (Tripoli 11.65 vs Benghazi 10.73 LYD,
-- 440 LYD of returns over 90 days) and is worth having, but the user chose to
-- defer it — "keep cost, we will fix it later for precise calculations".
--
-- MEASURED, all-markets scope (the heaviest path):
--   v2: shared hit=18327
--   v3: shared hit=18459   (+132 buffers, +0.7% — the carrier_live scan)
--
-- Buffers are the metric that matters here and they are deterministic: both
-- EXPLAIN runs reported hit=18459 byte-for-byte, all cache hits, zero disk
-- reads. Wall-clock on this shared instance is NOT reliable — six consecutive
-- calls measured 86.9 / 57.1 / 57.0 / 56.7 / 57.0 / 79.6 ms (median ~57 ms)
-- while two EXPLAIN runs minutes apart reported 72.5 ms and 1172 ms with
-- IDENTICAL buffer counts. Same pages touched, 16x the time: that spread is
-- CPU contention on the shared dev instance, not query cost. Do not read the
-- v2 "109.1 ms" and this "~57 ms" as a speed-up; both are noise around the
-- same ~18k-buffer plan. Still one round-trip.
-- ============================================================
