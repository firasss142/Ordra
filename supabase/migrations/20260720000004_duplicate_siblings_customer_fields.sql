-- 20260720000004_duplicate_siblings_customer_fields.sql
-- Adds customer_name / customer_address / customer_city to each duplicate
-- sibling returned by get_duplicate_orders_batch. The popover card now shows
-- the customer name + address instead of the order reference. Recreates the
-- function from 20260720000003_duplicate_siblings_product_image.sql plus three
-- columns in the matches CTE and siblings payload. Signature, market-isolation
-- guard, matching rules and GRANT are unchanged.
