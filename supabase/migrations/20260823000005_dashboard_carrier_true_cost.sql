-- ============================================================
-- 20260823000005_dashboard_carrier_true_cost.sql
-- Dashboard round-4b: the carrier cost column becomes a real number.
--
-- THE PROBLEM. `Coût / livraison` rendered 10 LYD for BOTH Libyan carriers,
-- because it was delivery_cost / delivered and delivery_fee is a flat
-- per-carrier rate. A column that returns the same value for every row cannot
-- inform a decision; it was spending space to say nothing.
--
-- THE FIX. Carry carriers.return_fee through hist_carrier and sum it, so the
-- app can express what a SUCCESSFUL delivery actually costs once the failures
-- are paid for:
--
--     (delivered x delivery_fee + returned x return_fee) / delivered
--
-- Measured over 90 days: Tripoli 11.65 LYD vs Benghazi 10.73 — an 8.6% real
-- penalty that the flat figure hid entirely, plus 440 LYD burned on returns.
--
...
-- already used by docs/business-logic.md ("Per carrier:
-- count_of_returned_orders x carriers.return_fee"), by
-- lib/calculations/business-profitability.ts, and by this function's own
-- markets_money CTE. The dashboard must not invent a second cost model.
--
-- The ratio itself is derived in TypeScript (mapRpcPayload), not here, matching
-- the module rule that the RPC returns raw counts and sums while every rate is
-- computed in one place alongside the denominator it came from.
--
-- Signature unchanged -> CREATE OR REPLACE, no DROP window.
-- ============================================================
