-- 20260720000005_customer_history_customer_name.sql
-- Adds customer_name to each order in get_customer_history_detail (address and
-- city are already returned). The repeat-buyer popover card now shows the
-- customer name + address instead of the order reference. Recreates the
-- function from 20260720000002_customer_history_product_image.sql plus one
-- field in the orders payload. Signature, match logic, lead/stats aggregation
