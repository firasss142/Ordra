-- ============================================================
-- 20260823000003_dashboard_health_v2.sql
-- Dashboard round-2: the page loses its period selector, so the rollup gains
-- the fields that replace it.
--
-- WHY EACH ADDITION:
--
--   today / trailing7  The period tabs are gone; "is today normal?" is now
--                      answered inside the KPI tiles instead. The baseline is
--                      the TRAILING 7-DAY mean, not the 30-day mean, because
--                      the Libya market restarted: 186 of the last 187 orders
--                      arrived in one week. Against the 30-day mean a normal
--                      day reads +594% — an artefact of the restart, not a
--                      signal. Against the 7-day mean the same day reads +62%.
--
--   committed          Revenue committed but not yet realised. Without it the
--                      dashboard shows 1 184 LYD of revenue while 453 orders
--                      sit at 'uploaded', which reads as a collapse rather than
--                      as lag. Money is realised at delivery (see the v1 header)
--                      and that will not change; this exposes the pipeline
--                      instead of distorting the revenue definition.
--
--   daily.confirmed    Feeds the flow chart's confirmation line. NOTE this is
--                      EVENT-based while the sibling outcome columns on the same
--                      row are COHORT-based. They are plotted as different marks
--                      (columns vs line) and labelled separately precisely
--                      because they are not the same kind of count.
--
--   image_url          Product rows now show the product image.
--
--   p_carrier_from     Carrier donuts read a 90-day window; delivery rates need
--                      volume to be worth drawing, and 30 days of a restarted
--                      market yields n<10 per carrier.
--
-- The signature changes, so the v1 function is DROPped first — adding a
-- parameter would otherwise leave a 7-arg overload behind that PostgREST could
-- still resolve to, silently serving stale shapes.
--
-- MEASURED, all-markets scope (the heaviest path), 6834 orders / 24849 history:
--   v1: Execution Time  76.3 ms, shared hit=12051
--   v2: Execution Time 109.1 ms, shared hit=18327
-- The ~33 ms is the 90-day carrier window; it buys carrier rates with enough
-- volume to draw honestly (Tripoli n=322, Benghazi n=63 — at 30 days both were
-- under n=15 and would have rendered as "n=…" rather than a rate). Still one
-- round-trip, still two orders of magnitude under the 0.5-4.2 s the pre-RPC
-- path recorded in 20260822000002. No new index was needed.
-- ============================================================

DROP FUNCTION IF EXISTS get_dashboard_health(UUID, DATE, DATE, DATE, DATE, DATE, DATE);
...
BEGIN
  ...
  hist_carrier AS (
    SELECT h.status_to, h.order_id, o.carrier_id, c.name, c.delivery_fee
    FROM order_history h
    JOIN orders   o ON o.id = h.order_id
  ...
  INTO v_result
  FROM money m, ad, funnel f, flow fl, today_orders tord, today_events tev, committed cm;
