-- ============================================================
-- 20260620000003_no_answer_allow_confirmed.sql
-- Companion to 20260620000002 (relaxed transition_order_status).
-- The /no-answer endpoint calls no_response_with_auto_reject(...)
-- which has its OWN hardcoded source-status guard. Without this
-- migration, the relaxed graph permits confirmed → attempt_*, but
-- the RPC rejects with "invalid no-answer from status confirmed"
-- before the graph is consulted.
--
-- There are two overloads of the RPC (one without p_actor_type,
-- one with). Both must be patched to allow 'confirmed' as a
-- valid source. Bodies are reproduced verbatim from the live
-- definitions; only the IN (...) check is widened.
-- ============================================================

-- ---- Overload 1: 4-arg (no p_actor_type) ----
...
BEGIN
  ...
  -- Allow 'confirmed' so agents can backtrack a mistakenly-confirmed order.
  IF v_current_status NOT IN (
    'assigned', 'attempt_1', 'attempt_2', 'attempt_3',
...
-- ---- Overload 2: 5-arg (with p_actor_type) ----
CREATE OR REPLACE FUNCTION public.no_response_with_auto_reject(
  p_order_id uuid,
  p_next_attempt order_status,
  p_callback_at timestamp with time zone DEFAULT NULL::timestamp with time zone,
