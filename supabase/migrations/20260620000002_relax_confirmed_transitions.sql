-- ============================================================
-- 20260620000002_relax_confirmed_transitions.sql
-- Allow agents/managers to backtrack a confirmed order to
-- attempt_*, callback_scheduled, or rejected. The previous graph
-- only allowed confirmed → uploaded | dispatch_scheduled | deleted,
-- which left no path for honest mistakes (customer recanted,
-- agent confirmed by accident).
--
-- Side-data integrity (attempts_count, rejection_reason, callback_at)
-- is enforced by the calling endpoints (/no-answer, /callback,
-- /reject), not by the function — so changing the graph alone is
-- safe.
--
-- Full body reproduced from 20260506000000_uploaded_status_model.sql
-- with only the 'confirmed' branch changed.
-- ============================================================
