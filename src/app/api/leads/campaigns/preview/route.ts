...
export async function POST(req: NextRequest) {
  ...
  if (filter_json.product_id) query = query.eq("product_id", filter_json.product_id);
  if (filter_json.city) query = query.eq("customer_city", filter_json.city);
  ...
  return NextResponse.json({ data: { matched: count ?? 0 } });
}
