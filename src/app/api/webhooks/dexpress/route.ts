...
import {
  handleDexpressWebhook,
  type DexpressWebhookDeps,
  type DexpressOrderLookup,
} from "@/lib/dexpress-webhook-handler";
import type { OrderStatus } from "@/types/order-status";

export async function POST(request: NextRequest) {
  const expectedToken = process.env.DEXPRESS_WEBHOOK_SECRET ?? "";
  ...
  const deps: DexpressWebhookDeps = {
    ...
    findOrderByTracking: async (tracking) => {
      const { data } = await admin
        ...
        .limit(1)
        .maybeSingle();
      return (data as DexpressOrderLookup | null) ?? null;
    },
  ...
  try {
    const result = await handleDexpressWebhook({ token, rawBody, deps });
    return NextResponse.json(result.body, { status: result.status });
  } catch (err) {
    console.error("dexpress webhook: unhandled exception", err);
