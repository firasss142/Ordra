...
export async function GET(req: NextRequest) {
  ...
  const scopeMarket =
    actor.role !== "super_admin" ? actor.market_id : null;
  const includePrints = q.kind === "all" || q.kind === "print";
  const includeScans =
    q.kind === "all" || q.kind === "scan" || q.kind === "return";

  // 2x factor only needed when both streams are active and either could fill the page.
  ...
  const printsPromise = (async () => {
    ...
    let qb = supabase
      .from("label_prints")
      .select(
        "id, order_id, is_reprint, created_at, orders(order_number, customer_name, customer_city)",
      )
  ...
  const scansPromise = (async () => {
    ...
    let qb = supabase
      .from("inventory_log")
      .select(
        "id, order_id, reason, created_at, is_damaged, products!inner(market_id, name), orders(order_number, customer_name, customer_city)",
      )
  ...
  // Merge, apply cursor tie-break, sort desc.
  const merged = [...printRows, ...scanRows]
    .filter((r) => {
      if (!cursor) return true;
      if (r.at < cursor.createdAt) return true;
      if (r.at > cursor.createdAt) return false;
      // Same timestamp: enforce strict ordering via (kind, id)
      const rowKind: WarehouseHistoryKindCursor =
        r.kind === "print" ? "print" : "scan";
    ...
    .sort((a, b) => {
      if (a.at === b.at) {
        const ak = a.kind === "print" ? "print" : "scan";
        const bk = b.kind === "print" ? "print" : "scan";
  ...
  if (merged.length > q.limit) {
    ...
    if (last) {
      nextCursor = encodeWarehouseHistoryCursor({
        createdAt: last.at,
        kind: last.kind === "print" ? "print" : "scan",
        id: last.id,
      });
    }
  }
