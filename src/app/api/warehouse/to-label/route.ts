...
export async function GET(req: NextRequest) {
  ...
  // Confirmed orders in own market that don't yet have a label_prints row.
  // Join products so the client can render low-stock badges without a second fetch.
  let query = supabase
    .from("orders")
    .select(
  ...
  return NextResponse.json({ orders: pending }, { headers: cacheHeaders });
}
