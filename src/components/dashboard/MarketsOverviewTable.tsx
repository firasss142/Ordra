...
export function MarketsOverviewTable() {
  const router = useRouter();
  const params = useParams<{ locale: string }>();

  const { data: marketsData } = useSWR<{ data: Market[] }>("/api/markets");
    ...
    >
      <table style={{ width: "100%", borderCollapse: "collapse" }}>
        ...
        <tbody>
          {markets.map((market) => {
            ...
            return (
              ...
              >
                ...
                <td style={{ ...tdStyle, fontFamily: "monospace", color: "#6D7175" }}>
                  {market.code.toUpperCase()}
                </td>
                <td style={tdStyle}>
                  <span style={{ color: market.is_active ? "#008060" : "#6D7175", fontSize: 16 }}>●</span>
                </td>
                ...
                <td style={{ ...tdStyle, textAlign: "end", fontVariantNumeric: "tabular-nums" }}>
                  {m?.confirmation_rate != null ? `${m.confirmation_rate.toFixed(1)}%` : "—"}
                </td>
                <td style={{ ...tdStyle, textAlign: "end", fontVariantNumeric: "tabular-nums" }}>
                  {m?.agents_active ?? "—"}
                </td>
                <td style={tdStyle}>
                  <button
                    onClick={() => router.push(`/${params.locale}/dashboard?market_id=${market.id}`)}
                    style={{
                      background: "none",
                      border: "none",
                      color: "#2C6ECB",
          ...
          {markets.length === 0 && (
            <tr>
              <td colSpan={8} style={{ ...tdStyle, textAlign: "center", color: "#6D7175", padding: 32 }}>
