...
export function AgentDrilldown({ agentId, agentName, marketId, onClose }: AgentDrilldownProps) {
  ...
  const { data: metricsData } = useSWR<{ data: { rejection_breakdown: RejectionRow[] } }>(metricsUrl, fetcher);
      ...
      >
        ...
        >
          <span style={{ fontSize: 16, fontWeight: 600, color: "#1A1A1A" }}>
            {agentName}
            {!loading && (
              <span style={{ fontWeight: 400, color: "#6D7175", marginLeft: 6 }}>
        ...
        {/* Scrollable body */}
        <div style={{ flex: 1, overflowY: "auto", paddingBottom: selected.size > 0 ? 72 : 0 }}>
          {loading && (
                ...
                >
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    ...
                    <span style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 13, color: "#6B7280" }}>
                      <span style={{ color: statusDotColor(order.status), fontSize: 10 }}>●</span>
        ...
        {/* Rejection breakdown */}
        {metricsData?.data?.rejection_breakdown && metricsData.data.rejection_breakdown.length > 0 && (
          <div
            style={{
              padding: "16px 24px",
          ...
          >
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              {metricsData.data.rejection_breakdown.map((row) => (
                <div key={row.reason} style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <div style={{ width: 110, fontSize: 12, color: "#374151", flexShrink: 0 }}>
                  ...
                  <div style={{ fontSize: 12, color: "#6B7280", minWidth: 40, textAlign: "end" }}>
        ...
        {/* Bottom bar — visible when ≥1 selected */}
        {selected.size > 0 && (
          <ReassignControls
