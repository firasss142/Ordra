...
- **Phase 2 — Connexions.** Tabbed workspace: Vue d'ensemble (hub + automations), Storefronts (rebuild token-clean + Archive/Delete UI), Transporteurs (rebuild + Archive/Delete UI), Services tiers (Meta + Sheets + WhatsApp placeholder), Correspondances (+ edit/delete). Small backend: `PATCH`/`DELETE /api/mappings/products/[id]`.
- **Phase 3 — Marchés & Journaux.** Marchés rebuilt (status banners, chevron funnels, edit modal) + `PATCH /api/markets/[id]`; Journaux → 4 tabs (Synchronisations + Audit + date-range), remove false retention note.
- **Cross-phase finish:** collapse nav fully to 4 items + all redirects once all four workspaces exist.
